...
def Rec():
    Rec = st.button("🎤 Speak", help="Speak to Ej's Assistant", disabled=True, key='3213', use_container_width=True)
...
with st.sidebar:
	...
	if dropdown_menu == 'Ejs Assistant':
		st.write(':orange[Ej Mode]:green[ Enabled]')
...
with tab1:
	...
	res_box.markdown(f":blue[Ej's Assistant:  ]")
	...
	if ok:
		if selected:
			...
			for resp in openai.Completion.create(model='text-davinci-003',
					...
					res_box.markdown(f":blue[Ej's Assistant:  ]:green[*{result}*]")
			if ok & selected2:
				speech = BytesIO()
				speech_ = gTTS(
					text=result, 
			...
			if ok & selected2:
				...
				speech_.write_to_fp(speech)
				st.audio(speech)
			history.append("Ej's Assistant: " + result)
	...
	if uploaded_file is not None:
		...
		#with st.sidebar:
		res_box.write(":blue[Ej's Assistant:  ]" + f':green[{result}]')
		st.markdown("----")
		st.write(df)
		st.markdown("----")
		st.download_button('Save Response', result)
